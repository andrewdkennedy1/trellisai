  ...
  <script type="module">
    ...
    function App() {
      ...
      async function runAgent() {
        ...
        try {
          ...
          setRunState("ready");
          setTurns(function(current) {
            const sprinkler = body && body.sprinkler_plan;
            const sprinklerText = sprinkler
              ? " I also checked recent field history and queued " + (sprinkler.zone || sprinkler.controller || "the smart sprinkler zone") + " to move from " + (sprinkler.before && sprinkler.before.start_time ? sprinkler.before.start_time : "6:30 AM") + " to " + (sprinkler.after && sprinkler.after.start_time ? sprinkler.after.start_time : "5:15 AM") + " for approval."
              : "";
            return [
              {
                role: "agent",
                title: "Agent response",
                body: (body.answer || "The agent prepared tomorrow's plan.") + sprinklerText
              }
            ].concat(current).slice(0, 5);
          });
          await refreshState();
          ...
          h("div", { className: "stage-board" },
            ...
            h(AnimatePresence, { mode: "popLayout" },
              (thoughts.length ? thoughts : [
                ...
                "Receipts show what changed: note saved, history checked, sprinkler schedule edited, jobs queued."
    ...
    function ReceiptRail(props) {
      ...
      const fallback = [
        ...
        { label: "Looked up field history", detail: "Ready to compare this note with similar field days.", state: "watch" },
    ...
    function SprinklerPanel(props) {
      ...
      return h(motion.section, {
        ...
        h("div", { className: "panel-title" },
          ...
          h("span", { className: props.sprinklerApproved ? "state-chip" : sprinkler ? "approval-chip" : "state-chip" },
            props.sprinklerApproved ? "approved" : sprinkler ? "ready for approval" : "waiting on agent"
          )
        ),
